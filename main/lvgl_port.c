...
static lv_disp_t *display_init(esp_lcd_panel_handle_t panel_handle)
{
    ...
    buf2 = heap_caps_malloc(buffer_size * sizeof(lv_color_t), LVGL_PORT_BUFFER_MALLOC_CAPS); // Allocate second buffer for double buffering
    assert(buf1 && buf2); // Ensure allocation succeeded
    ESP_LOGI(TAG, "LVGL buffer size: %dKB x2", buffer_size * sizeof(lv_color_t) / 1024); // Log buffer size
#endif /* LVGL_PORT_AVOID_TEAR_ENABLE */

    // Initialize LVGL draw buffers
    lv_disp_draw_buf_init(&disp_buf, buf1, buf2, buffer_size); // Initialize the draw buffer (double buffering)

    ESP_LOGD(TAG, "Register display driver to LVGL");
    lv_disp_drv_init(&disp_drv); // Initialize the display driver
    disp_drv.hor_res = LVGL_PORT_H_RES; // 800
    disp_drv.ver_res = LVGL_PORT_V_RES; // 480
    disp_drv.sw_rotate = 1; // Enable software rotation
    disp_drv.rotated = LV_DISP_ROT_90; // Rotate 90 degrees (portrait)
    // Do NOT set full_refresh, not compatible with sw_rotate
...
esp_err_t lvgl_port_init(esp_lcd_panel_handle_t lcd_handle, esp_lcd_touch_handle_t tp_handle)
{
    ...
    // No LVGL software rotation: keep native landscape
